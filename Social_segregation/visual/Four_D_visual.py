...
extreme_cities = set()
for theme in theme_columns:
    highest_city = ranked_data.loc[ranked_data[f"{theme}_rank"].idxmin(), "City"]
    lowest_city = ranked_data.loc[ranked_data[f"{theme}_rank"].idxmax(), "City"]
...
# 生成城市排名映射字典，并按照排名排序
city_rank_map = {city: sorted_data.loc[sorted_data["City"] == city, "City_Rank"].values[0] for city in extreme_cities}
...
# 设置颜色
np.random.seed(42)
colors = {city: np.random.rand(3, ) for city in ranked_data["City"]}
