...
def relative_importance_analysis(city_list):
    ...
    print('shap coefficients:')
    explainer = shap.Explainer(model, X)
    shap_values = explainer(X)
    shap_values_mean = np.abs(shap_values.values).mean(axis=0)
    shap_importance = shap_values_mean / np.sum(shap_values_mean) * 100
    plt.figure(figsize=(8, 6))
    bars = plt.bar(feature_names, shap_importance, color=['blue', 'green', 'red', 'purple'])

    # 在柱子上方显示百分比数值
    for bar, percent in zip(bars, shap_importance):
        plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{percent:.2f}%',
                 ha='center', va='bottom', fontsize=12)

    plt.xlabel("Themes")
    plt.ylabel("SHAP Importance (%)")
    plt.title("SHAP Importance Analysis")
    plt.show()
